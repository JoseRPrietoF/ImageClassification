...
for folder_orig in "${folders[@]}"; do
    ...
    for folder2 in "${folders[@]}"; do
        if ! grep -q "$folder2" <<< "$folder_orig" ; then
            ...
            path_save_results=/home/jose/projects/image_classif/acts/results/AHPC/1folder/JMBD_tr${folder_orig}_te${folder2}_${model}_${decoding}
    ...
    if [ "$check" = true ] ; then
        ...
        python get_error_classif.py --path_hyp ${faux} --path_gt ${fauxgt} --folders ${te_folders} --from_decoder True --only_error False
        rm $faux
        rm $fauxgt
    fi
    # exit
