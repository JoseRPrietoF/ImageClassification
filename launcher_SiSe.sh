...
for model in "${models[@]}"; do
    ...
    mkdir -p ${work_dir}
    python main.py --model ${model} --learning_rate 0.001 --batch_size ${batch_size} --epochs ${epochs} --corpus ${corpus} --width ${width} --img_dirs ${img_dirs} --n_classes 4 --work_dir ${work_dir}
done